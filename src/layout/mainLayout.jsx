...
const MainLayout = ({ children }) => {
  ...
  const hapticTab = () =>
    fire({
      haptic: [{ duration: 30 }, { delay: 60, duration: 40, intensity: 1 }],
      sound: true,
    });
  const { user, userType, permissions, logout,credits } = useAuth();
  const config = layoutConfig[userType] || layoutConfig.admin;
  const routes =
    userType === "employee" ? getEmployeeRoutes(permissions) : config.routes;
  ...
  const activeTabIndex = isCompany
    ? getCompanyActiveIndex()
    : localActiveTabIndex;
  ...
  const userEmail =
    user?.email || user?.companyEmail || user?.adminEmail || "User";
    ...
    >
      ...
      >
            ...
            <nav className="relative flex items-center rounded-[10px] p-1.25 gap-0">
              ...
              <div
                ...
                style={{
                  ...
                  backgroundImage:
                    "linear-gradient(to bottom, #bef264 0%, #a3e635 45%, #84cc16 100%)",
                  boxShadow:
        ...
        <div className="relative flex items-center gap-4" ref={userMenuRef}>
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-700 rounded-full shadow-sm text-sm font-medium">
          ...
          <button
            ...
            style={{
              boxShadow:
                "0 2px 6px rgba(0,0,0,0.16), 0 4px 12px rgba(0,0,0,0.12), inset 0 1px 0 rgba(255,255,255,0.06)",
            }}
          >
            {userInitial}
          </button>
          ...
          {showUserMenu && (
            <div
              className="absolute right-0 mt-3 bg-white rounded-lg z-50 overflow-hidden min-w-max"
              style={{
                boxShadow:
            ...
            >
              <div className="px-5 py-4 border-b border-slate-100 bg-slate-50">
                ...
                <p
                  className="text-[13px] font-semibold text-slate-900 wrap-break-words max-w-xs"
                  title={userEmail}
                >
                  {userEmail}
                </p>
          ...
          >
            <div
              ...
              style={{
                ...
                boxShadow:
                  "0 20px 60px rgba(0, 0, 0, 0.15), 0 0 1px rgba(0, 0, 0, 0.08), inset 0 1px 0 rgba(255, 255, 255, 0.8)",
              }}
            >
              {/* Sidebar header */}
              <div
                ...
                style={{
                  backgroundImage:
                    "radial-gradient(circle, rgba(180, 190, 175, 0.4) 2.5px, transparent 2.5px)",
      ...
      {/* Main content */}
      <main
        className="flex-1 overflow-auto px-6 py-3 bg-[#f6f8f4]"
